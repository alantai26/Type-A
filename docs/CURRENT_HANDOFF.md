...
- **`predict(model, user_id, category)`** — the inverse of `build_features`. A full prediction is `X @ coef + intercept`, but a single row of X is all zeros except three 1.0s, so every other term multiplies out and it collapses to three dict lookups and an add. That's why inference needs no matrix math and no sklearn.
- **Missing columns contribute `0.0` via `.get(name, 0.0)`, and that fallback is correct at every level.** Unknown user → all three lookups miss → global mean (plus the category term, which still applies — an unseen user asking about Cafe gets 6.63, not the bare 6.45 intercept). Known user, unrated category → the user coefficient still lands, so the prediction sits on *that user's* baseline. Verified: Alan's never-rated Dessert cell returns 8.05, not 6.45. No special-case code produces that; it falls out of shrinkage.
- **Artifact is JSON, and `load_json` gates on `model_version`.** A stale artifact doesn't crash — it serves plausible, outdated numbers forever. Failing loudly at startup is the whole point of the gate. `coefficients/atomic_v1.json` is 4.7 KB / 59 coefficients and **is committed**: Render deploys from git, has no object storage, and the free tier has no pre-deploy command, so there is nowhere else for it to come from. It scales as `n_users × 6` — ~1 MB of git churn per retrain at 10k users, at which point it needs object storage. Not this ticket.
- **`scripts/train_atomic.py`** — the CLI that strings the library together. `--dry-run` skips the write, `--out` overrides the path. Prints alpha, matrix shape, skipped off-taxonomy rows, and a per-user prediction table.
- **⚠️ The smoke test earned its keep immediately.** A first pass at `predict()` looked up `f"user={id} | cat={cat}"` (spaces around the pipe) while `build_features` emits `f"user={id}|cat={cat}"`. Ruff passed, nothing raised, and `.get`'s default silently swallowed it — the entire personalization term evaluated to 0.0 and Coffee Snob's Cafe prediction came back **6.49 instead of 9.39**, a number too plausible to notice by eye. What caught it was a domain check: *every* user's top category was `Dessert`. A "do predictions vary by user?" check would NOT have caught it — the user term alone still varies people while leaving every user's preference *order* identical. The guard now asserts that favorites differ across users.
- **This is the argument for TYP-75 being a real ticket** rather than an afterthought: the failure mode here was invisible to the linter, the type checker, and casual inspection.

### 2026-08-21 — TYP-74 + TYP-70 shipped: seed-data hygiene + Atomic Recommender foundation

- **Merged in PR #37** (commit `8111c42`, merge `0caea52`). Branch `typ-70-mvml-3-backend-atomic-recommender-ridge-regression`. Closes TYP-74 and TYP-70.
- **⚠️ Linear didn't auto-close TYP-74** — the commit message said only `TYP-70 seed fixes`, so TYP-74 sat at Todo despite its code being in `dev`. Closed by hand on 2026-08-22. When one PR covers two tickets, the body needs a `Fixes TYP-N` line for *each*.
- **TYP-70 was rescoped from one 8pt ticket into five** once implementation started, because 8pt hid five separable deliverables: **TYP-74** (mvML-2.5, seed hygiene), **TYP-70** (mvML-3a, data loading + feature encoding + ridge fit — both in PR #37), **TYP-77** (mvML-3b, `predict()` + JSON artifact + training script), **TYP-75** (mvML-3c, eval harness), **TYP-76** (mvML-3d, wire `/places/{id}/predict`).
