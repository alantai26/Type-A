...
class OutingInvitation(Base):                                                                                                                     
    ...
    __table_args__ = (
        CheckConstraint("rsvp_status IN ('pending', 'accepted', 'rejected')", name="check_outing_invitation_status"),
    )

    user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.user_id"), primary_key=True)                                               
    outing_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("outings.outing_id"), primary_key=True)
